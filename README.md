# ArquiteturaBaseWeb - Template Web .NET 10
...
```json
{
  "identity": "ArquiteturaBaseWeb.Template",
  "name": "Arquitetura Base Web .NET 10",
  "shortName": "baseweb",
  "sourceName": "ArquiteturaBaseWeb"
}
```

Ao gerar uma aplicação com outro nome, o .NET substitui `ArquiteturaBaseWeb` pelo novo nome nos arquivos, no projeto e nos namespaces.

### Instalar o template localmente

Abra o terminal na raiz deste repositório, onde estão `.template.config` e `ArquiteturaBaseWeb.csproj`, e execute:
...
Assim, a `ArquiteturaBaseWeb` pode ser usada como ponto de partida para novas aplicações Web .NET 10 sem copiar arquivos manualmente.
